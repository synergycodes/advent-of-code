	...
	lineReader.on("close", () => {
		...
		input.forEach(({ firstPoint, secondPoint }) => {
			if (firstPoint[0] === secondPoint[0]) {
				...
				const biggerX = Math.max(firstPoint[0], secondPoint[0]);
				let smallerX = Math.min(firstPoint[0], secondPoint[0]);
				const biggerY = Math.max(firstPoint[1], secondPoint[1]);
				let smallerY = Math.min(firstPoint[1], secondPoint[1]);

				while (smallerX <= biggerX && smallerY <= biggerY) {
					const point = diagram[smallerX][smallerY];
					if (point === ".") {
						diagram[smallerX][smallerY] = 1;
					} else {
						diagram[smallerX][smallerY]++;
					}
					smallerX++;
					smallerY++;
				}
