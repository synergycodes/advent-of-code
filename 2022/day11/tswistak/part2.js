...
// to avoid bigints, we can just do modulo over LCM of "test" values
// all are prime numbers so their LCM is their product
const modulo = monkeys.reduce((acc, monke) => acc * monke.test, 1);

for (let i = 0; i < 10000; i++) {
  for (let j = 0; j < monkeys.length; j++) {
    for (let k = 0; k < monkeys[j].items.length; k++) {
      let newWorryLevel = monkeys[j].operation(monkeys[j].items[k] % modulo);
